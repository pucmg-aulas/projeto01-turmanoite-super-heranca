...
public class GerenciarReservaController {
    ...
    private void carregarItensDaReserva() {
        ...
        if (selectedRow >= 0) {
            ...
            for (var bebida : reserva.getComanda().getBebidas()) {
                model.addRow(new Object[]{bebida.getId(), bebida.getNome(), "Bebida", bebida.getPreco()});
            }
        }
    }
    ...
    private void removerItem() {
        ...
        if (selectedRow >= 0) {
            UUID itemId = (UUID) this.view.getTableItens().getValueAt(selectedRow, 0);
            ...
            if (itemTipo.equals("Prato")) {
                reserva.getComanda().getPratos().removeIf(prato -> prato.getNome().equals(itemId));
            } else if (itemTipo.equals("Bebida")) {
                reserva.getComanda().getBebidas().removeIf(bebida -> bebida.getId().equals(itemId));
            }
